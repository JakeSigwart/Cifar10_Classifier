...
with graph.as_default():
	...
	with tf.device("/device:GPU:0"):
		...
		h_norm3C = batch_normalize(h_conv3C, n_out=64, phase_train=Training_status)
		...
		h_pool3D = tf.nn.max_pool(h_actv3D, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool3D')
...
with tf.Session(graph=graph) as sess:
	...
	image_batch, hot_label_batch = cifar.get_random_batch(batch_size, one_hot=True)
		acc = sess.run([accuracy], feed_dict={data: image_batch, labels: hot_label_batch, Training_status: False})
